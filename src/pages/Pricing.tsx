...
const subscriptionPlans = [{
  ...
  yearlyPrice: 0,
  monthlyPriceId: null,
  // No checkout for free tier
  yearlyPriceId: null,
  credits: 100,
  icon: Sparkles,
  features: ["100 AI credits (one-time)", "Community support", "Basic features"],
  popular: false,
  isFree: true
}, {
  id: "basic-sub",
  ...
  credits: 100,
  icon: Zap,
  features: ["100 AI credits/month", "Email support", "Basic analytics", "1 user seat"],
  popular: false
}, {
  id: "starter-sub",
  name: "Starter",
  ...
  credits: 100,
  icon: Rocket,
  features: ["100 AI credits/month", "Priority email support", "Advanced analytics", "2 user seats"],
  popular: false
}, {
  id: "pro-sub",
  name: "Pro",
  ...
  credits: 200,
  icon: Star,
  features: ["200 AI credits/month", "Priority support", "Custom integrations", "5 user seats", "API access"],
  popular: true
}, {
  id: "business-sub",
  name: "Business",
  ...
  credits: 400,
  icon: Building,
  features: ["400 AI credits/month", "Dedicated support", "White-label options", "Unlimited seats", "SLA guarantee"],
  popular: false
}];
...
const setupPackages = [{
  ...
  icon: Calendar,
  features: ["AI Booking Agent", "Calendar integration", "3-day testing window", "Basic setup support"],
  popular: false
}, {
  id: "call",
  ...
  icon: Phone,
  features: ["AI Call Agent", "24/7 inbound handling", "Lead capture", "Call summaries"],
  popular: false
}, {
  id: "combo",
  name: "Booking + Call",
  ...
  icon: Zap,
  features: ["AI Call Agent", "AI Booking Agent", "Phone number setup", "Go-live testing", "Priority support"],
  popular: false
}, {
  id: "full",
  ...
  icon: Globe,
  features: ["AI Call Agent", "AI Booking Agent", "1-page conversion website", "Complete automation", "Dedicated onboarding"],
  popular: true
}];
  ...
  const handleCustomCheckout = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke("create-custom-payment", {
        body: {
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
              ...
              <div className="flex items-center gap-3 bg-muted/50 px-4 py-2 rounded-full">
                <Label htmlFor="billing-toggle" className={`text-sm cursor-pointer ${!isYearly ? 'font-semibold text-foreground' : 'text-muted-foreground'}`}>
                  Monthly
                </Label>
                ...
                {isYearly && <span className="bg-accent text-accent-foreground text-xs font-semibold px-2 py-0.5 rounded-full ml-1">
                    Save 10%
                  </span>}
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 lg:grid-cols-5 gap-4">
              ...
              return <Card key={plan.id} className={`relative bg-card border-border/50 transition-all hover:border-primary/50 ${plan.popular ? "ring-2 ring-primary" : ""}`}>
                    ...
                    <CardHeader className="text-center pb-4">
                      ...
                      <div className="mt-3">
                        ...
                        {!isFree && <span className="text-muted-foreground text-sm ml-1">
                            /{isYearly ? "year" : "month"}
                          </span>}
                      </div>
                      {isYearly && !isFree && plan.monthlyPrice > 0 && <p className="text-xs text-accent font-medium mt-1">
                    ...
                    <CardContent className="space-y-3">
                      <ul className="space-y-1.5">
                        {plan.features.map(feature => <li key={feature} className="flex items-start gap-2 text-xs">
                            <Check className="w-3.5 h-3.5 text-accent shrink-0 mt-0.5" />
                            {feature}
                          </li>)}
                      </ul>
                      {isFree ? <Button className="w-full" variant="outline" asChild>
                          <a href="/login">Sign Up Free</a>
                        </Button> : <Button className="w-full" variant={plan.popular ? "default" : "outline"} onClick={() => priceId && handleCheckout(priceId, plan.id)} disabled={loadingId === plan.id || !priceId}>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              const Icon = pkg.icon;
              return <Card key={pkg.id} className={`relative bg-card border-border/50 transition-all hover:border-primary/50 ${pkg.popular ? "ring-2 ring-primary" : ""}`}>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <Button className="w-full" variant={pkg.popular ? "default" : "outline"} onClick={() => handleCheckout(pkg.priceId, pkg.id)} disabled={loadingId === pkg.id}>
            ...
            <Card className="bg-gradient-to-br from-primary/5 to-accent/5 border-border/50">
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <Input type="number" placeholder="Enter amount (min $50)" value={customAmount} onChange={e => setCustomAmount(e.target.value)} min={50} className="text-lg" />
                </div>
                <p className="text-xs text-muted-foreground text-center">
                  Minimum $50. We'll reach out to discuss your custom requirements.
                </p>
                <Button className="w-full" onClick={handleCustomCheckout} disabled={loadingId === "custom" || !customAmount}>
